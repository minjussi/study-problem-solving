...
def solution(bridge_length, weight, truck_weights):
    ...
    current_weight = 0 # 이걸 기준으로 빼고 더하고를 계속 해서 weight와 비교를 해야함
    
    while bridge:
        time += 1
        passed_truck = bridge.popleft()
        current_weight -= passed_truck # 지나간 건 빼고 !
        
        if truck_weights:
            if current_weight + truck_weights[0] <= weight: # 비교 로직
                next_truck = truck_weights.popleft()
                bridge.append(next_truck)
                current_weight += next_truck # 무게를 더해 주어야 함 !!
            else:
                bridge.append(0)
